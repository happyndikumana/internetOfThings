...
//                if(!restartTimer(t1TimerHandler))
        ...
        case DHCP_OFF:
            ...
            //reset internet info
            uint8_t dataZeros[4] = {0,0,0,0};
            dhcpSendMessage(ether, DHCPRELEASE);
            writeEeprom(1, 0); // turn dhcp off
            etherSetIpAddress(dataZeros);
            etherSetIpSubnetMask(dataZeros);
            etherSetIpGatewayAddress(dataZeros);
            etherSetIpDnsAddress(dataZeros);
            etherSetIpTimeServerAddress(dataZeros);
            dhcpSetState(DHCP_DISABLED);
            break;
    }
}
