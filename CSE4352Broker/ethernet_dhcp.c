...
// Subroutines                
//-----------------------------------------------------------------------------
bool sendMessageRequest = false;
bool sendFinMessage = false;
...
//                mqttConnectFlag
                ...
                {
                    sendMessageRequest = false; //to ensure this is false
                    sendFinMessage = true;
                }
            ...
            if (strcmp(token, "end") == 0) {
                token = strtok(NULL, " ");
                uint8_t octate = asciiToUint8(token);
                tcpEndConnectionCommand(octate);
            }
...
//    dhcpSendMessage(data, 1);
    ...
    etherSetIpAddress(ipFrom);

//    while(1)
//        listenForEther(data);
//    while(1);

    getDnsData(data, ipFrom);
//    tcpClientSendMessage(data, 2);
...
//    while(1);
    ...
    while (true)
    {
        ...
        if(dataInSocketFlag) {
            sendTcpPendingMessage(data);
        }

        //send tcp client message
        if(sendMessageRequest)
        {
            sendMessageRequest = false; //reset flag
            tcpSendMessage(data, false);
        }

        //send client fin
        else if(sendFinMessage)
        {
            tcpSendMessage(data, true);
            sendFinMessage = false;
        }
...
//                        etherSendUdpResponse(data, (uint8_t*)"Received", 9);
                    ...
                    if(etherIsTcp(data)) {
                        if(dhcpGetState() == DHCP_BOUND){
                            if(serverFlag)
                                processTcpServerMessage(data);
                            else if(clientFlag)
